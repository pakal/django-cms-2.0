...
class UtilsTestCase(LocaleurlTestCase):
    ...
    def test_locale_url(self):
        ...
        for script_name in ["", "/mountpoint", "/mountpoint/"]:
            ...
            self.assertEqual('/dummy/test/independent/bla/bla',
                    utils.locale_url('/test/independent/bla/bla', 'en', '/dummy'))
            self.assertEqual('/test/independent/bla/bla',
                    utils.locale_url('/test/independent/bla/bla', 'en', ''))
...
class MiddlewareTestCase(LocaleurlTestCase):
    ...
    def test_script_name_handling(self):
        r1 = self.request_factory.get('/nl-be/test/independent/?foo=bar', SCRIPT_NAME="/mnt")
        r2 = self.middleware.process_request(r1)
        self.assertEqual(301, r2.status_code)
        self.assertEqual('/mnt/test/independent/?foo=bar', r2['Location'])

        r1 = self.request_factory.get('/nl-be/test/dependent/?foo=bar', SCRIPT_NAME="/mnt")
        r2 = self.middleware.process_request(r1)        
        self.assertEqual(None, r2)      
...
class DefaultPrefixMiddlewareTestCase(MiddlewareTestCase):
    ...
    def test_script_name_with_default_prefix(self):
        r1 = self.request_factory.get('/test/dependent/?foo=bar', SCRIPT_NAME="/mnt")
        r2 = self.middleware.process_request(r1)        
        self.assertEqual('/mnt/en/test/dependent/?foo=bar', r2['Location'])

    def test_ignored_request(self):
        r1 = self.request_factory.get('/test/dependent/?foo=bar', SCRIPT_NAME="/mnt")
        r1.LANGUAGE_CODE = "en" # as if anothre middleware had already set it
        r2 = self.middleware.process_request(r1)        
        self.assertEqual(None, r2)        
...
class NoDefaultPrefixMiddlewareTestCase(MiddlewareTestCase):
    ...
    def test_script_name_with_no_default_prefix(self):
        r1 = self.request_factory.get('/test/dependent/?foo=bar', SCRIPT_NAME="/mnt")
        r2 = self.middleware.process_request(r1)        
        self.assertEqual(None, r2)
...
class TagsTestCase(LocaleurlTestCase):
    ...
    def test_locale_url_tag(self):
        for script_name in ["", "/mountpoint", "/mountpoint/"]:
            ...
            self.assertEqual(script_name+'/en/dummy/', self.render_template(
                    '{% locale_url "en-us" dummy0 %}'))
    
            self.assertEqual(script_name+'/fr/dummy/4', self.render_template(
                '{% locale_url "fr" dummy1 test=4 %}'))
    
            self.assertEqual(script_name+'/en/dummy/4', self.render_template(
    ...
    def test_chlocale_filter(self):
        for script_name in ["", "/mountpoint", "/mountpoint/"]:
            ...
            if script_name:
                self.assertRaises(ValueError, self.render_template, 
                    '{{ "/dummy/"|chlocale:"fr" }}') # must begin with same script prefix
                 
            self.assertEqual(script_name+'/fr/dummy/', self.render_template(
                    '{{ "%s/dummy/"|chlocale:"fr" }}'%script_name))
    
            self.assertEqual(script_name+'/en/dummy/', self.render_template(
                    '{{ "%s/fr/dummy/"|chlocale:"en-gb" }}'%script_name))
    ...
    def test_rmlocale_filter(self):
        ...
        for script_name in ["", "/mountpoint", "/mountpoint/"]:
            ...
            if script_name:
                self.assertRaises(ValueError, self.render_template,
                    '{{ "/dummy/"|rmlocale }}') # must begin with same script prefix
                
            self.assertEqual(script_name+'/dummy/', self.render_template(
                    '{{ "%s/dummy/"|rmlocale }}'%script_name))
    
            self.assertEqual(script_name+'/dummy/', self.render_template(
                    '{{ "%s/nl-be/dummy/"|rmlocale }}'%script_name))
    
            self.assertEqual(script_name+'/dummy/', self.render_template(
                    '{{ "%s/en/dummy/"|rmlocale }}'%script_name))
...
class SitemapTestCase(LocaleurlTestCase):
    ...
    def test_localeurl_sitemap(self):
        ...
        sitemap = DummySitemap('en-gb')
        self.assertEqual(sitemap.get_urls()[0]['location'],
                         'http://www.example.com/en/dummy/0/')
